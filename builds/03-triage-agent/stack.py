...
from agent import MAX_STEPS, TOKEN_BUDGET
from config import MODEL
from dispatch import TOOLS
from eutils import CACHE_DIR
...
INVENTORY: dict[str, tuple[str, str]] = {
    ...
    "tools": row(
        TOOL_NAMES,
        "arguments are validated at the dispatch boundary and a rejection is "
        "written to the trace rather than returned as prose.",
    ),
    "working_memory": row(
        f"one messages list per record screened, bounded at {MAX_STEPS} steps "
        f"and {TOKEN_BUDGET} tokens",
        "no verdict is carried between records: each starts from the criteria "
        "file.",
    ),
    "episodic_memory": row(
        f"the E-utilities response cache at {CACHE_DIR}",
        "keyed by record and it survives runs. TRIAGE_CACHE_DIR moves it.",
    ),
    "reference_memory": row(
        "criteria/repurposing_v3.yaml, versioned, and the fixture corpus",
        "the criteria version is stamped on every verdict, so a criteria "
        "change shows up in the output rather than being inferred later.",
    ),
    "orchestration": row(
        "agent loop over the tools, with one model call per record in "
        "screen.py",
        "the model decides which tool; the code decides which record.",
    ),
    "trace": row(
        "JSONL, one event per line, runs/<run_id>.jsonl",
        "written as the run happens rather than assembled at the end.",
    ),
}
